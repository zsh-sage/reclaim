...
Compact instruction file for Reclaim — AI-native expense reimbursement platform (FastAPI + Next.js).

> **Push target:** All feature branches and PRs must be pushed to the `fork` remote (`FilbertNg/reclaim_backup`) only. Do not push to `origin` (`zsh-sage/reclaim`).

> **Container runtime:** This repo uses `docker` / `docker compose` commands, but the developer uses **Podman**. Substitute `docker` → `podman` and `docker compose` → `podman-compose` (or `podman compose` if using the Docker-compatible CLI) everywhere below.
...
**Entry point:** `backend/main.py` — FastAPI app, mounts all routers under `/api/v1/`, health check at `/health`.
