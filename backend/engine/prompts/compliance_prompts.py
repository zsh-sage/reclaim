...
2. Determine overall_judgment:
   - APPROVE: Every line item is APPROVED, no significant risks.
   - REJECT: Every line item is REJECTED.
   - PARTIAL_APPROVE: A mix of statuses (at least one APPROVED and at least one REJECTED or PARTIAL_APPROVE).
   - MANUAL REVIEW: Conflicting signals, HIGH human-edit risk on key items, or unable to determine with confidence.
...
Return ONLY a valid JSON object with exactly this structure (no markdown, no extra text, no code fences):
{{
  "overall_judgment": "APPROVE | REJECT | PARTIAL_APPROVE | MANUAL REVIEW",
